...
- **Observability** — Micrometer counters for every significant operation, exposed via `GET /actuator/metrics`. Key metrics: `aetherledger.transactions.posted/completed/failed/reversed`, `aetherledger.reconciliation.matched/mismatched`, `aetherledger.outbox.published` (tagged by `eventType`), `aetherledger.outbox.publish.failures`, `aetherledger.webhooks.delivered/failed` (tagged by `eventType`), `aetherledger.scheduled.reconciliation.processed/skipped/failed`. Health endpoint at `GET /actuator/health`
